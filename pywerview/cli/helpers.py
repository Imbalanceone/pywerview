...
def get_netuser(domain_controller, domain, user, password=str(), lmhash=str(),
                ...
                custom_filter=str(), attributes=tuple(), queried_sid=str()):
    requester = NetRequester(domain_controller, domain, user, password,
                             lmhash, nthash, do_kerberos, do_tls,
                             user_cert, user_key)
    return requester.get_netuser(queried_username=queried_username,
                                 ...
                                 attributes=attributes, queried_sid=queried_sid)
...
def get_netcomputer(domain_controller, domain, user, password=str(),
    ...
    return requester.get_netcomputer(queried_computername=queried_computername,
                                     ...
                                     custom_filter=custom_filter, attributes=attributes,queried_sid=queried_sid)

def get_template(domain_controller, domain, user, password=str(), lmhash=str(),
                 nthash=str(), do_kerberos=False,
                 resolve_sids=False, queried_guids=None, do_tls='', name="", queried_domain=str()):
    requester = NetRequester(domain_controller, domain, user, password,
                             lmhash, nthash, do_kerberos, do_tls)

    return requester.get_template(queried_guids, name, queried_domain=queried_domain)

def get_container(domain_controller, domain, user, password=str(), lmhash=str(),
                  nthash=str(), do_kerberos=False,
                  resolve_sids=False, queried_guids=None, do_tls='', name="", queried_domain=str()):
    requester = NetRequester(domain_controller, domain, user, password,
                             lmhash, nthash, do_kerberos)

    return requester.get_container(queried_guids, name, queried_domain=queried_domain)
